...
class BridgingTests: XCTestCase {
    ...
    func test1() {
        ...
        let input = after(interval: 0).then{ _ in 1 }
    ...
    func test2() {
        ...
        AnyPromise(input).then { obj -> AnyPromise in
            XCTAssertEqual(obj as? Int, 1)
            return AnyPromise(after(interval: 0).then{ _ in 2 })
        }.then { obj -> Void  in
    ...
    func test3() {
        ...
        AnyPromise(input).then { obj -> Promise<Int> in
            XCTAssertEqual(obj as? Int, 1)
            return after(interval: 0).then{ _ in 2 }
        }.then { value -> Void  in
            XCTAssertEqual(value, 2)
            ex.fulfill()
        }
    ...
    func test4() {
        ...
        Promise(value: 1).then { _ -> AnyPromise in
            return AnyPromise(after(interval: 0).then{ _ in 1 })
        }.then { x -> Void in
    ...
    func test5() {
        ...
        Promise(value: 1).then { _ -> AnyPromise in
            let promise = after(interval: 0.1).then{ _ in throw Error.dummy }
