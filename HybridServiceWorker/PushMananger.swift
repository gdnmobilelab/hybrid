...
@objc class PushManager : NSObject, PushManagerExports {
    ...
    static func listenForDeviceToken() {
        self.deviceTokenListener = SharedResources.ApplicationEvents.on("didRegisterForRemoteNotificationsWithDeviceToken", { token in
            ...
            return Promise(value: ())
                .then {
                    ...
                    return Promise<PushSubscription> { fulfill, reject in
                        SharedResources.ApplicationEvents.once("didRegisterForRemoteNotificationsWithDeviceToken", { _ in
    ...
    func _getSubscription() -> PushSubscription? {
        if PushManager.deviceToken == nil {
            ...
            let appName = SharedResources.appBundle.bundleIdentifier!
