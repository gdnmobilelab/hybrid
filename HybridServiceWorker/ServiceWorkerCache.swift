                ...
                let downloadAndStoreTasks = urlsAsNSURLs.map { (url: URL) -> Promise<RequestAndResponse> in
                    ...
                    return GlobalFetch.fetch(request: fetchRequest)
                        .then { response in
                            ...
                            .then { data in
                                return Promise(value: RequestAndResponse(request: fetchRequest, response: response, data: data))
                            }
                    ...
                    .then { reqResponsePairs in
                        ...
                        try Db.mainDatabase.inTransaction({ (db) in
                            ...
                            for r in reqResponsePairs {
                                ...
                                try db.executeUpdate("INSERT INTO cache (service_worker_url, cache_id, resource_url, contents, headers, status) VALUES (?,?,?,?,?,?)", values: [self.serviceWorkerURL.absoluteString, self.name, r.request.url, r.data, headersAsJSON, r.response.status])
                            }
